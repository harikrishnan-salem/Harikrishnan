...
window.addEventListener('resize', () => {
  ...
  window._bgResize = setTimeout(resize, 120);
});
...
function drawGradient(t) {
  ...
  for (let i = 0; i < gradientNodes.length; i++) {
    ...
    const gx = (0.15 + 0.7 * (i / (gradientNodes.length - 1))) +
      Math.sin(t * 0.00013 * (i + 1)) * 0.06;

    const gy = 0.18 + Math.cos(t * 0.00011 * (i + 2)) * 0.06;

    const gr = Math.min(1.0, n.r *
      (0.9 + 0.15 * Math.sin(t * 0.00009 * (i + 3))));
